...
- Extract a helper function or use conditional class building with `cn()` from `@/lib/utils`:
  ```typescript
  const baseClasses = 'aspect-square flex items-center justify-center rounded-lg p-1 text-center text-xs font-medium transition-colors'
  const unclaimedClasses = 'bg-white border border-gray-200 text-gray-900 active:bg-gray-50 cursor-pointer'
  ```
